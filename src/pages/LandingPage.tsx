...
/** Ancho y respiración compartidos por todas las secciones. */
...
function SiteHeader() {
  return (
    <header className="sticky top-0 z-40 border-b border-line bg-white">
      <div className={`${SHELL} flex items-center justify-between gap-6 py-[15px]`}>
        <Link to="/" className="flex items-center gap-2" aria-label="MediConnect — inicio">
          <img src="/logo.png" alt="" className="h-[30px] w-auto" />
          <span className="font-display text-xl text-brand-deep">MediConnect</span>
        </Link>

        <nav className="flex items-center gap-1">
          {/* En pantalla chica sobreviven solo las dos acciones de sesión. El
              catálogo no se pierde: el CTA del hero, justo debajo, lleva ahí.
              Un menú hamburguesa sería inventar algo que el diseño no define. */}
  ...
  return (
    <footer className="border-t border-white/[0.08] bg-abyss text-on-night-soft">
      <div className={`${SHELL} grid gap-10 pb-10 pt-[60px] lg:grid-cols-[1.4fr_1fr_1fr_1fr]`}>
        <div>
          <div className="font-display text-[26px] text-white">MediConnect</div>
